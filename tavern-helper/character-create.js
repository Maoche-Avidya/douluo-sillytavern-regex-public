...
(function () {
  ...
  const PAGE_INDEX = 1;
  const BUILD_ID = "character-create@3.0+0a0996da643d";
  ...
  const CHAT_LIFECYCLE_SCAN_POLICIES = Object.freeze({
    CHAT_CHANGED: [0, 240, 1600],
    CHARACTER_FIRST_MESSAGE_SELECTED: [0, 240],
    USER_MESSAGE_RENDERED: [80, 240],
    CHARACTER_MESSAGE_RENDERED: [80, 240],
    MESSAGE_UPDATED: [120],
    MESSAGE_SWIPED: [0, 240],
    SIGNATURE_CHANGED: [80, 240],
    DEFAULT: [80],
  });
  ...
  const state = {
    ...
    performance: {
      ...
      queueFlushCount: 0,
      queueCandidatesProcessed: 0,
      totalQueueDurationMs: 0,
      maxQueueDurationMs: 0,
  ...
  function scanNew(options = {}) {
    ...
    state.scanRuns += 1;
    if (options.root) state.performance.scopedScanRuns += 1;
  ...
  function flushQueue() {
    ...
    const queueDurationMs = Math.max(0, performanceNow() - flushStartedAt);
    state.performance.queueFlushCount += 1;
    state.performance.queueCandidatesProcessed += nodes.length;
    state.performance.lastQueueDurationMs = queueDurationMs;
    state.performance.totalQueueDurationMs += queueDurationMs;
    state.performance.maxQueueDurationMs = Math.max(state.performance.maxQueueDurationMs, queueDurationMs);
  }
  ...
  function lifecycleScanDelays(eventKey) {
    return CHAT_LIFECYCLE_SCAN_POLICIES[eventKey]
      || CHAT_LIFECYCLE_SCAN_POLICIES.DEFAULT;
  }

  function scheduleLifecycleScans(eventKey, reason) {
    if (state.disposed) return;
    ...
    const delays = Array.from(new Set(lifecycleScanDelays(eventKey).map((delay) => Math.min(delay, remainingWait))));
    delays.forEach((delay) => {
      let timer = 0;
      timer = scheduleTracked(() => {
        timers.delete(timer);
      ...
      CHAT_LIFECYCLE_EVENT_KEYS.forEach((key, index) => {
        lifecycleEventNames(record.eventTypes, key, index).forEach((eventName) => {
          addLifecycleSubscription(
            source,
            eventName,
            () => scheduleLifecycleScans(key, `${entry.label}:${eventName}`),
            entry.label
          );
        });
      });
  ...
  function checkChatSignature() {
    ...
    if (state.lastChatSignature && next !== state.lastChatSignature) {
      ...
      scheduleLifecycleScans("SIGNATURE_CHANGED", "chat-signature-changed");
    }
