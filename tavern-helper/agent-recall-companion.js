...
(function () {
  ...
  function saveLastSelection(scopeKey, era, result) {
    ...
    selections[`${scopeKey}::${era || "unknown"}`] = {
      schemaVersion: 2,
      savedAt: Date.now(),
      ...
      currentChapter: result && result.chapterRoute && Number(result.chapterRoute.number) || null,
    };
    storageSet(LAST_SELECTION_KEY, selections);
  }
  ...
  async function synchronizeEraScope(controlRead) {
    ...
    const explicitChapter = Core.inferCurrentChapterFromContext(storyContext(), era.era);
    const currentChapter = explicitChapter && explicitChapter.number || previous && previous.currentChapter || null;
    ...
    await applyFinalGreenlights({
      ...
      tk: ordinary.tk,
      chapterEntries,
      chapterRoute: currentChapter ? { number: currentChapter, source: explicitChapter ? explicitChapter.source : "previous_valid_chapter" } : null,
...
@media (max-width:${MOBILE_BREAKPOINT}px){
  ...
  const api = { version: VERSION, buildId: BUILD_ID, enable, disable, resume, status, auditSkills, upgradeSkills, cancelCurrentRun, diagnose, mount, dispose };
